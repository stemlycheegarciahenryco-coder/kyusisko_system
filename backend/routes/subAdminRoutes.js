  ...
  try {
    const result = await pool.query(
      `SELECT 
        ...
        sub_email, contact_number, is_active, status, -- Added status here
        region, city, barangay, street_address, website,provider_type, proof_files
        proof_files
       FROM sub_admins 
       ORDER BY status = 'pending' DESC, id DESC` // Changed order to show pending first
    );
...
router.post('/register-organization', uploadFields, async (req, res) => {
  const { 
  ...
  // 1. Correctly extract files from the object structure
...
if ((provider_type === "LGU" || provider_type === "NGO") &&
  ...
  return res.status(400).json({ error: "Proof and SEC Certificate are required." });
}
  try {
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(sub_password, salt);

    // 2. Updated to 16 placeholders (make sure your SQL matches this)
    await pool.query(
      ...
      [
        ...
        false,         // is_active
        'pending',     // status
        JSON.stringify(fileData), // proof_files (The JSON object)
        provider_type
      ]
    );
...
router.post('/reject/:id', verifyToken, async (req, res) => {
  try {
    ...
    // Update status to rejected and save the reason for the compliance page view
    await pool.query(
      `UPDATE sub_admins 
    ...
    // Pass the ID explicitly to the email service so it builds the link
...
// 2. NEW ENDPOINT: GET ORG REJECTION DETAILS (For the compliance page to load dynamically)
...
// 3. NEW ENDPOINT: COMPLY/RESUBMIT REQUIREMENTS WITH NEW FILES
router.post('/comply/:id', uploadFields, async (req, res) => {
  ...
  try {
    // Check provider type to confirm files are accurately present
    ...
    // Reset status to pending, overwrite files array block, wipe old rejection reason
    await pool.query(
      `UPDATE sub_admins 
       SET status = 'pending', 
           proof_files = $1,
...
router.post('/request-otp', async (req, res) => {
    ...
    try {
        // Check if email is already in sub_admins
        ...
        // Match your table: otp_codes (email, code, method, expires_at)
        await pool.query(
            `INSERT INTO otp_codes (email, code, method, expires_at, created_at) 
             ...
             ON CONFLICT (email) DO UPDATE SET 
                ...
                created_at = NOW()`,
            [email, generatedCode, 'org_reg'] // method 'org_reg' separates this from students
        );
...
router.post('/verify-otp', async (req, res) => {
    ...
    try {
        ...
        // Clean up: Optional - delete code after success so it can't be reused
