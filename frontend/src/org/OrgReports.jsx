...
export default function OrgReports() {
    ...
    const createDocumentReport = () => {
            ...
            ...(fundData || []).map(p => {
                ...
                return new TableRow({
                    children: [
                        ...
                        new TableCell({ children: [new Paragraph(p.total_budget != null ? `₱${p.total_budget.toLocaleString()}` : 'Not set')] }),
                        new TableCell({ children: [new Paragraph(`₱${(p.disbursed || 0).toLocaleString()}`)] }),
                        new TableCell({ children: [new Paragraph(p.remaining_budget != null ? `₱${p.remaining_budget.toLocaleString()}` : '—')] }),
        ...
        const children = [
            ...
            new Paragraph({
                children: [new TextRun({ text: "Per-Program Breakdown:", bold: true })],
            }),
            ...
            new Paragraph({
                children: [new TextRun(financialInterpretation || "")],
            }),
            new Paragraph({}),
        ...
        (DEMOGRAPHIC_TABS || []).forEach(({ key, label }) => {
            ...
            if (rows.length === 0) {
                children.push(
                    new Paragraph({
                        children: [new TextRun("(no records)")],
                    })
                );
            } else {
                rows.forEach((row) => {
                    children.push(
                        new Paragraph({
                            children: [
                                new TextRun(
                                    `  • ${row.name}: ${row.total} total (Male ${row.male}, Female ${row.female}${row.unspecified ? `, Unspecified ${row.unspecified}` : ''})`
                                ),
                            ],
                        })
                    );
        ...
        children.push(
            new Paragraph({
                children: [new TextRun(demographicInterpretation || "")],
            })
        );
    ...
    const createPdfReport = () => {
        ...
        const fundTableBody = [
            ...
            ...(fundData || []).map(p => {
                ...
                return [
                    p.title || p.program,
                    p.total_budget != null ? `₱${p.total_budget.toLocaleString()}` : 'Not set',
                    `₱${(p.disbursed || 0).toLocaleString()}`,
                    p.remaining_budget != null ? `₱${p.remaining_budget.toLocaleString()}` : '—',
                    STATUS_LABELS[status] || status
                ];
            })
        ];

        // Build demographic content blocks
        ...
        const docDefinition = {
            content: [
                ...
                { text: `Programs With No Budget Set: ${totals.programsMissingAmount}`, margin: [0, 0, 0, 10] },
        ...
        return {
            docDefinition,
            filename: `scholarship-report-${fileTimestamp}.pdf`
        };
    };
                ...
                <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 pb-3">
                    {DEMOGRAPHIC_TABS.map(({ key, label, icon: Icon }) => (
                        <button
                            ...
                            className={`flex items-center gap-1.5 text-xs font-extrabold px-3.5 py-2 rounded-lg transition-all ${demoTab === key
                                ? 'bg-blue-600 text-white'
