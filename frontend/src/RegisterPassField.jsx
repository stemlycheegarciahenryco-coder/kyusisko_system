...
const REQUIREMENT_DEFS = [
    ...
    { rules: ["lowercase"], label: "One Lowercase" },
...
export default function RegisterPassField({ name, value = "", onChange, placeholder, error, showStrength = false }) {
    ...
    const failedRules = value
        ? passwordSchema.validate(value, { list: true })
        : ["min", "max", "uppercase", "lowercase", "digits", "symbols"];
    ...
    const calculateStrength = (pass) => {
        ...
        switch (score) {
            ...
            case 4: return { score: 80, label: "Good", color: "bg-[#093fb4]" };
            case 5: return { score: 100, label: "Strong", color: "bg-emerald-500" };
            default: return { score: 0, label: "Invalid", color: "bg-red-500" };
        }
    };

    const strength = showStrength ? calculateStrength(value) : { score: 0, label: "Empty", color: "bg-slate-200" };

    // Determine if border should be red (either via explicitly passed error or if error exists on empty)
        ...
        <div className="flex flex-col w-full">
            <div className="relative group">
                ...
                <input
                    ...
                    className={`w-full pl-12 pr-12 py-3.5 bg-white/60 border-2 rounded-2xl outline-none transition-all placeholder:text-black/30 font-bold text-slate-900 text-base shadow-sm ${
                        isInvalid
                            ? "border-[#FF1E1E] focus:border-[#FF1E1E] bg-red-50/20 text-[#FF1E1E]"
                            : "border-white/80 focus:bg-white focus:border-[#093fb4]"
                    }`}
                />
                <button
                    type="button"
