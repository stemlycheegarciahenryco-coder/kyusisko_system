...
import { Camera, User, MapPin, Phone, Plus, X, Award, FileText, GraduationCap, BookOpen, Shield, Edit2 } from 'lucide-react';
import api from '../api';
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FFFCFB] flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-[11px] font-black text-[#093fb4] uppercase tracking-widest">Loading Profile</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#FFFCFB] pt-20 pb-16">
      <div className="max-w-3xl mx-auto px-5">
        ...
        <div className="flex items-center gap-3 mb-7">
          ...
          <span className="text-[10px] font-black text-[#093fb4] uppercase tracking-[0.3em]">Academic Profile</span>
        ...
        <div className="bg-white border border-[#093fb4]/10 rounded-2xl mb-5 overflow-hidden shadow-sm">
          ...
          <div className="p-7 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-[#093fb4] text-xs font-bold tracking-widest mb-4">
                {student?.student_email}
              </p>
              {/* Degree + School row */}
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-[#093fb4] bg-[#093fb4]/8 border border-[#093fb4]/15 px-3 py-1.5 rounded-full">
                  <GraduationCap size={12} />
                  {degreeName}
                </span>
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-slate-600 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-full">
                  <BookOpen size={12} />
                  {schoolName}
                </span>
              </div>
            ...
            <div className="flex-shrink-0">
              <button
                ...
                className="flex items-center gap-2 text-[11px] font-black uppercase bg-[#093fb4] text-white px-4 py-2.5 rounded-xl hover:bg-[#093fb4]/90 transition-all shadow-md active:scale-95"
              >
        ...
        <div className="bg-white border border-[#093fb4]/10 rounded-2xl p-6 mb-5 shadow-sm">
          <SectionLabel icon={<Shield size={12} />} title="Personal Information" />
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
            <InfoItem icon={<Phone />} label="Contact" value={student?.scontact_number} />
            <InfoItem icon={<MapPin />} label="Address" value={[student?.sstreet, student?.sbarangay].filter(Boolean).join(', ') || "Not provided"} />
        ...
        <div className="bg-white border border-[#093fb4]/10 rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-5">
            <div className="flex-1">
              <SectionLabel icon={<FileText size={12} />} title="Student Portfolio" />
            </div>
            <button
              onClick={() => setIsPortfolioModalOpen(true)}
              className="flex items-center gap-1.5 text-[10px] font-black uppercase bg-[#093fb4]/10 text-[#093fb4] px-3 py-2 rounded-xl hover:bg-[#093fb4] hover:text-white transition-all shadow-sm active:scale-95"
            >
              ...
              student.portfolio_data.map((item, idx) => (
                ...
                >
                  <div className="w-9 h-9 rounded-lg bg-[#093fb4]/8 border border-[#093fb4]/15 flex items-center justify-center flex-shrink-0 group-hover:bg-[#093fb4] transition-colors">
                    <Award size={15} className="text-[#093fb4] group-hover:text-white transition-colors" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[9px] font-black uppercase text-[#093fb4] tracking-widest mb-0.5">{item.type}</p>
...
function SectionLabel({ icon, title }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-black text-[#093fb4] uppercase tracking-[0.2em]">{title}</span>
...
function EmptyState({ text }) {
  return (
    <p className="text-center text-slate-400 text-xs py-6 italic font-medium">{text}</p>
  );
}
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        <p className="text-sm font-bold text-black leading-snug mt-0.5">{value || "Not provided"}</p>
      </div>
    </div>
  );
}